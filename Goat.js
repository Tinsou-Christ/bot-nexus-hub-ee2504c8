...
// MULTI-BOT: chaque bot deploye depuis le panneau web recoit ses propres fichiers
// de session (config, configCommands, account) via ces variables d'environnement.
// Sans elles, on retombe sur le comportement d'origine.
const dirConfig = path.normalize(process.env.GOATBOT_CONFIG || `${__dirname}/config${['production', 'development'].includes(NODE_ENV) ? '.dev.json' : '.json'}`);
const dirConfigCommands = path.normalize(process.env.GOATBOT_CONFIG_COMMANDS || `${__dirname}/configCommands${['production', 'development'].includes(NODE_ENV) ? '.dev.json' : '.json'}`);
const dirAccount = path.normalize(process.env.GOATBOT_ACCOUNT || `${__dirname}/account${['production', 'development'].includes(NODE_ENV) ? '.dev.txt' : '.txt'}`);
