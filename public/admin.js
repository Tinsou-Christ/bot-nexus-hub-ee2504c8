...
async function botAction(action, uid) {
  await api(`/admin/bot/${action}`, { method: "POST", body: JSON.stringify({ uid }) });
  refresh();
}

function renderBots(bots) {
  const box = $("adminBots");
  box.innerHTML = bots.length ? "" : '<p style="color:#9b8a8e">Aucune session active.</p>';
  bots.forEach((bot) => {
...
async function loadLogs() {
  ...
  $("adminLogs").textContent = (data.logs || []).join("\n") || "Aucun log.";
...
setInterval(() => { if (token && !$("console").classList.contains("hidden")) refresh(); }, 5000);
