...
/**
 * Applies the auto font styling (like Cassidy: bold via **, italic via *, mono via backticks)
 * to whatever is about to be sent, whether it's a plain string or a
 * message-form object ({ body, attachment, ... }).
 * @param {string|object} form
 * @returns {string|object}
 */
function applyAutoFont(form) {
	try {
		if (typeof form === "string") {
			return fonts.auto(form);
		}
		if (form && typeof form === "object" && typeof form.body === "string") {
			return { ...form, body: fonts.auto(form.body) };
		}
	} catch (err) {
		log.err("applyAutoFont", "Failed to apply auto font styling", err);
	}
	return form;
}
	...
	return {
		send: async (form, callback) => {
			try {
				...
				return await api.sendMessage(applyAutoFont(form), event.threadID, callback);
			}
			catch (err) {
		...
		reply: async (form, callback) => {
			try {
				global.statusAccountBot = 'good';
				return await api.sendMessage(applyAutoFont(form), event.threadID, callback, event.messageID);
			}
			catch (err) {
				if (JSON.stringify(err).includes('spam')) {
					setErrorUptime();
