...
module.exports = {
        ...
        onStart: async function ({ message, args, usersData, event, getLang }) {
                ...
                switch (args[0]) {
                        ...
                        case "-a": {
                                if (args[1]) {
                                        ...
                                        for (const uid of uids) {
                                                if (config.premium.includes(uid))
                                                        premiumIds.push(uid);
                                                else
                                                        notPremiumIds.push(uid);
                                        }

                                        config.premium.push(...notPremiumIds);
                        ...
                        case "-r": {
                                if (args[1]) {
                                        ...
                                        for (const uid of premiumIds) {
                                                config.premium.splice(config.premium.indexOf(uid), 1);
                        ...
                        case "list":
                        case "-l": {
                                const premiumList = await Promise.all(config.premium.map(async uid => {
                        ...
                        case "-c": {
                                ...
                                const name = await usersData.getName(uid);
                                const isPremium = config.premium.includes(uid);
                                const expireTime = await usersData.get(uid, "data.premiumExpireTime");
